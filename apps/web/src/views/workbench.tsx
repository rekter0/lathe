...
export function RevisionAssetOptions({ assets }: { assets: readonly AssetRevision[] }) {
  return <>{assets.map((asset) => (
    <option key={asset.id} value={asset.id}>{asset.name} · r{asset.revision}</option>
  ))}</>;
}
...
function ConfigInspector({ data, onChanged }: { data: WorkbenchData; onChanged(): void }) {
  ...
  return <div className="inspector-content">
    ...
    <div className="config-section"><h3>System prompt blocks <span>{draft.promptBlocks.filter((item) => item.enabled).length}/{draft.promptBlocks.length}</span></h3><div className="inline-add"><Select value={promptRevisionId} onChange={(event) => setPromptRevisionId(event.target.value)}><option value="">Add from library…</option><RevisionAssetOptions assets={prompts.data?.assets ?? []} /></Select><Button type="button" variant="secondary" aria-label="Add selected system prompt block" onClick={addPrompt} disabled={!promptRevisionId}><PlusIcon /></Button></div><PromptBlocksEditor blocks={draft.promptBlocks} onChange={(promptBlocks) => setDraft({ ...draft, promptBlocks })} /><details className="prompt-preview"><summary>Final compiled system prompt <span>{compiledPrompt.length} chars</span></summary><pre>{compiledPrompt || "No enabled system prompt blocks."}</pre></details></div>
    <div className="config-section"><h3>Tools <span>{draft.tools.filter((item) => item.enabled).length}</span></h3><div className="inline-add"><Select value={toolRevisionId} onChange={(event) => setToolRevisionId(event.target.value)}><option value="">Add from library…</option><RevisionAssetOptions assets={toolSpecs.data?.assets ?? []} /></Select><Button variant="secondary" onClick={addTool} disabled={!toolRevisionId}><PlusIcon /></Button></div>{draft.tools.map((tool, index) => <div className="tool-binding" key={`${tool.toolRevisionId}-${index}`}><label className="tool-toggle"><input type="checkbox" checked={tool.enabled} onChange={(event) => setDraft({ ...draft, tools: draft.tools.map((item, itemIndex) => itemIndex === index ? { ...item, enabled: event.target.checked } : item) })} /><span><strong>{tool.name}</strong><small>{tool.mode} · {tool.description}</small></span><Select value={tool.mode} onChange={(event) => setDraft({ ...draft, tools: draft.tools.map((item, itemIndex) => itemIndex === index ? { ...item, mode: event.target.value as typeof item.mode, implementationRevisionId: null, targetId: null, mcpServerId: null } : item) })}><option value="manual">manual</option><option value="mock">mock</option><option value="real">real</option><option value="mcp">MCP</option></Select></label>{(tool.mode === "real" || tool.mode === "mock") && <Select aria-label={`${tool.name} implementation`} value={tool.implementationRevisionId ?? ""} onChange={(event) => setDraft({ ...draft, tools: draft.tools.map((item, itemIndex) => itemIndex === index ? { ...item, implementationRevisionId: event.target.value || null } : item) })}><option value="">Select implementation…</option><RevisionAssetOptions assets={implementations.data?.assets ?? []} /></Select>}{tool.mode === "real" && <Select aria-label={`${tool.name} target`} value={tool.targetId ?? ""} onChange={(event) => setDraft({ ...draft, tools: draft.tools.map((item, itemIndex) => itemIndex === index ? { ...item, targetId: event.target.value || null } : item) })}><option value="">Local host</option><RevisionAssetOptions assets={targets.data?.assets ?? []} /></Select>}{tool.mode === "mcp" && <Select aria-label={`${tool.name} MCP server`} value={tool.mcpServerId ?? ""} onChange={(event) => setDraft({ ...draft, tools: draft.tools.map((item, itemIndex) => itemIndex === index ? { ...item, mcpServerId: event.target.value || null } : item) })}><option value="">Select MCP server…</option><RevisionAssetOptions assets={mcpServers.data?.assets ?? []} /></Select>}</div>)}</div>
...
function withoutPromptBlock(blocks: PromptBlock[], index: number): PromptBlock[] {
  return blocks.filter((_block, blockIndex) => blockIndex !== index).map((block, order) => ({ ...block, order }));
}

export function PromptBlocksEditor({ blocks, onChange }: { blocks: PromptBlock[]; onChange(blocks: PromptBlock[]): void }) {
  if (blocks.length === 0) return <p className="quiet">This session has no prompt blocks. Add one from a harness or library revision.</p>;
  return <>{blocks.map((block, index) => <div className="prompt-block" key={`${block.revisionId}-${index}`}>
    <div className="prompt-block-header">
      <label>
        <input type="checkbox" checked={block.enabled} onChange={(event) => onChange(blocks.map((item, itemIndex) => itemIndex === index ? { ...item, enabled: event.target.checked } : item))} />
        <span className="prompt-block-name">{block.name}</span>
      </label>
      <span className="reorder">
        <button type="button" aria-label={`Move ${block.name} up`} title="Move prompt block up" onClick={() => onChange(moveItem(blocks, index, index - 1))} disabled={index === 0}>↑</button>
        <button type="button" aria-label={`Move ${block.name} down`} title="Move prompt block down" onClick={() => onChange(moveItem(blocks, index, index + 1))} disabled={index === blocks.length - 1}>↓</button>
        <button type="button" className="prompt-remove" aria-label={`Remove ${block.name} from session draft`} title="Remove from this session draft" onClick={() => onChange(withoutPromptBlock(blocks, index))}><X size={12} /></button>
      </span>
    </div>
    <Textarea aria-label={`${block.name} prompt content`} value={block.content} rows={5} onChange={(event) => onChange(blocks.map((item, itemIndex) => itemIndex === index ? { ...item, content: event.target.value } : item))} />
  </div>)}</>;
}
